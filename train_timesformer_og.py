...
class CFG:
    ...
    train_aug_list = [
        ...
        A.ShiftScaleRotate(rotate_limit=360, shift_limit=0.15,
                           scale_limit=0.15, p=0.75),
        A.OneOf([
            A.GaussNoise(std_range=(10, 50)),
            A.GaussianBlur(),
            A.MotionBlur(),
        ], p=0.4),
...
def read_image_mask(fragment_id, start_idx=17, end_idx=43, CFG=CFG):
    ...
    for i in idxs:
        if os.path.exists(CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/layers/{i:02}.tif"):
            image = cv2.imread(
                CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/layers/{i:02}.tif", 0)
        else:
            image = cv2.imread(
                CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/layers/{i:02}.jpg", 0)
    ...
    fragment_mask = cv2.imread(
        CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/{fragment_id_}_mask.png", 0)
...
def worker_function(fragment_id, CFG):
    ...
    print("Reading fragment", fragment_id)
    try:
    ...
    # Iterate through the image in steps defined by stride size
    for a in y1_list:
        for b in x1_list:
            # Only process tiles that are fully within the fragment mask (no black/zero pixels)
            if not np.any(fragment_mask[a:a + CFG.tile_size, b:b + CFG.tile_size] == 0):
                # Process if this is validation data OR if the mask has some ink pixels (>0.05)
                if (fragment_id == CFG.valid_id) or (not np.all(mask[a:a + CFG.tile_size, b:b + CFG.tile_size] < 0.05)):
                    # Further divide each tile into smaller windows of size CFG.size
                    for yi in range(0, CFG.tile_size, CFG.size):
                        for xi in range(0, CFG.tile_size, CFG.size):
                            ...
                            # For training data, add windows to training sets
                            if fragment_id != CFG.valid_id:
                                train_images.append(image[y1:y2, x1:x2])
                                train_masks.append(mask[y1:y2, x1:x2, None])
                                # Verify window dimensions are correct
                                assert image[y1:y2, x1:x2].shape == (
                                    CFG.size, CFG.size, CFG.in_chans)

                            # For validation data, add unique windows to validation sets
                            if fragment_id == CFG.valid_id:
                                if (y1, y2, x1, x2) not in windows_dict:
                                    valid_images.append(image[y1:y2, x1:x2])
                                    valid_masks.append(
                                        mask[y1:y2, x1:x2, None])
...
def get_train_valid_dataset(fragment_ids=['20231210121321', '20231022170901', '20231106155351', '20231005123336', '20230820203112', '20230826170124', '20230702185753', '20230522215721', '20230531193658', '20230903193206', '20230902141231', '20231007101615', '20230929220926', 'recto', '20231016151000', '20231012184423', '20231031143850']):
    threads = []
    results = []
    for _ in range(len(fragment_ids)):
        results.append(None)
    ...
    for idx, fragment_id in enumerate(fragment_ids):
        thread = threading.Thread(
            target=thread_target, args=(idx, fragment_id))
...
class CustomDataset(Dataset):
    def __init__(self, images, cfg, labels, xyxys=None, transform=None):
        self.images = images
        self.cfg = cfg
        self.labels = labels
    ...
    def fourth_augment(self, image):
        ...
        image_tmp[..., start_paste_idx: start_paste_idx +
                  cropping_num] = image[..., crop_indices]
    ...
    def __getitem__(self, idx):
        if self.xyxys is not None:
            ...
            if self.transform:
                ...
                label = data['mask']
                label = F.interpolate(label.unsqueeze(
                    0), (self.cfg.size//16, self.cfg.size//16)).squeeze(0)
...
class RegressionPLModel(pl.LightningModule):
    def __init__(self, pred_shape, size=256, with_norm=False):
        ...
        self.loss_func = lambda x, y: 0.5 * \
            self.loss_func1(x, y)+0.5*self.loss_func2(x, y)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train/total_loss", loss1.item(),
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, (x1, y1, x2, y2) in enumerate(xyxys):
            self.mask_pred[y1:y2, x1:x2] += F.interpolate(y_preds[i].unsqueeze(
                0).float(), scale_factor=16, mode='bilinear').squeeze(0).squeeze(0).numpy()
            self.mask_count[y1:y2,
                            x1:x2] += np.ones((self.size, self.size))

        self.log("val/total_loss", loss1.item(),
                 on_step=True, on_epoch=True, prog_bar=True)
        return {"loss": loss1}

    def on_validation_epoch_end(self):
        self.mask_pred = np.divide(self.mask_pred, self.mask_count, out=np.zeros_like(
            self.mask_pred), where=self.mask_count != 0)
        wandb_logger.log_image(key="masks", images=[np.clip(
            self.mask_pred, 0, 1)], caption=["probs"])

        # reset mask
        self.mask_pred = np.zeros(self.pred_shape)
        self.mask_count = np.zeros(self.pred_shape)
...
for fid in fragments:
    ...
    valid_mask_gt = cv2.imread(
        CFG.comp_dataset_path + f"train_scrolls/{fragment_id}/{fragment_id}_inklabels.png", 0)
    ...
    wandb_logger = WandbLogger(
        project="vesivus", name=run_slug+f'timesformer_big6_finetune')
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ModelCheckpoint(filename=f'timesformer_wild16_{fid}_fr'+'{epoch}', dirpath=CFG.model_dir,
                            monitor='train/total_loss', mode='min', save_top_k=CFG.epochs)
        ],
    )
